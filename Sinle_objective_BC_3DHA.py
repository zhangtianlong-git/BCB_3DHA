...
import time
from Input_paramters import start_angle, ZRESO, THETA_XY_RESOLUTION, GRADIENT_Z_RESOLUTION, z_s, z_g, x_g, x_s, y_g, y_s, second_res, zres_in_astar, terrain, RADIUS, GRADIENTS, MIN_LEN_SLOPE, MIN_LEN_CURV, MIN_LEN_TAN, K_MIN, MAX_LEN_TAN, MAX_NEAR_GRADIENT, EXPLORE_RES, LEN_SPIRAL, SINGLE_OBJECTIVE
...
tmp_f = 100000000000
heapq.heappush(pq, (tmp_f, tmp_index, nstart))
num_iter= 0
output_nodes = []
...
while True:
    ...
    f, c_id, current = heapq.heappop(pq)
    num_iter += 1
    ...
    for neighbor in get_neighbors(current):
        ...
        tmp_ff = [neighbor.f_cost, neighbor.f_carbon]
        tmp_f_used = tmp_ff[SINGLE_OBJECTIVE-1]  # choose f1 or f2 as the single objective function
        if neighbor_index in closeset:
            continue
        if (neighbor_index not in closeset) or (closeset[neighbor_index] > tmp_f_used):
            heapq.heappush(pq, (tmp_f_used, neighbor_index, neighbor))
            closeset[neighbor_index] = tmp_f_used
    current.pe = None  # Release memory
    current.house_set = None  # Release memory

print(num_iter)
# # Output the alignment results to JSON
output_diction, filename = {}, 'Optimized_alignment-objective-' + str(SINGLE_OBJECTIVE) + ".json"
for i in range(len(output_nodes)):
    tmp_node, alignment_num = output_nodes[i], i+1
