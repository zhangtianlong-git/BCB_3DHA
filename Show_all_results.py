...
"""
The JSON file information for the route includes the following:
{
    "Alignment ID": {
        "x_cors": xxx, 
        "y_cors": xxx, 
...
def get_final_alignment_info(alignment_info):
    ...
    output = get_cost_and_tunnel_info(h_diffs, 'S', 0, 0, 0)
...
als_3 = load_dict('Optimized_alignments-600.json')

code_alignments = {}

als_dics = [als_1, als_2, als_3]
...
"""--------------------------------Plot the manual alignment and calculate the cost----------------------------------"""
get_final_alignment_info([m_xs, m_ys, m_zs, m_ts, m_ss])
...
plt.show()
### Plot manual solution
plot_an_alignment([m_xs, m_ys, m_zs, m_ts, m_ss, m_bpd_ss, m_bpd_zs, m_pi_x, m_pi_y, m_pi_r])
### Plot three Pareto solutions based on the corresponding IDs: Acost, Acarbon, and Abalance.
plot_an_alignment(alignments_updated[21])
plot_an_alignment(alignments_updated[106])
plot_an_alignment(alignments_updated[44])